...
This repository holds **EZCanvas**, an AI assisted study platform built on the Canvas LMS API for international students studying in English speaking countries: the running app, and the design and specification package it is built from. Your job is to keep building it milestone by milestone from that package. The README covers setup and scripts.
...
- Background jobs (Canvas and Ed sync, reminders) with a simple scheduler that can run as a cron route or a worker; keep the job code independent of the scheduler. Today the Canvas sync runs on a timer started in `instrumentation.ts`, guarded by a claim on the connection row.
...
- Dates and hours a student sees are computed in their time zone (`User.timeZone`) with `lib/week.ts`, never with the server's local time.
...
See `.env.example` and the Configuration table in the README. The essentials:

```
DATABASE_URL=
AUTH_SECRET=                 # signs sign in sessions
APP_SECRET=                  # for encrypting stored user tokens and API keys
APP_URL=                     # public address, used in calendar feed links
CANVAS_DEFAULT_BASE_URL=     # e.g. https://canvas.cornell.edu (user can override)
RESEND_API_KEY=              # sign in links, later group project reminder emails
```
