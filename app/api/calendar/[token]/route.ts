...
export async function GET(_request: Request, { params }: RouteContext<"/api/calendar/[token]">) {
  ...
  const user = await db.user.findUnique({ where: { calendarToken: token }, select: { id: true, name: true, timeZone: true } });
  if (!user) return new Response("Not found", { status: 404 });

  const body = icsFeed(`EZCanvas · ${user.name ?? "My courses"}`, await feedEvents(user.id, user.timeZone));
